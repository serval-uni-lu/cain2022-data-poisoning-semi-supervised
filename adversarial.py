...
def multiple_flips_accuracy_mlp(
    x_train, y_train_unlabeled, lp_model, x_test, y_test, n_flips, weights=None
):
    if weights is None:
        weights = lp_model.custom_graph(x_train, y=None)
    ...
    clf = SimpleMlp(random_state=2020, impl="tf", input_shape=(x_train.shape[1],))
    # clf = svm.SVC(verbose=True,max_iter=1000,gamma='auto')
    clf.fit(x_train, y_train, args={"epochs": 5, "verbose": 2})
    acc_mlp = clf.score(x_test, y_test)
    return acc_mlp
...
def multiple_flips_accuracy_sklearn(
    x_train,
    y_train_unlabeled,
    lp_model,
    x_test,
    ...
    n_labelled = np.array(np.where(y_train_unlabeled != -1)).shape[1]
    idx_to_flip = best_to_flip(n_flips, weights, n_labelled)
    acc_t, x_train, y_train = multiple_flips_training(
        x_train, y_train_unlabeled, lp_model, x_test, y_test, idx_to_flip
    )
...
def reduce_influence_random(x_train, y_train_unlabeled, random_seed, n_flips):
    n_labelled = np.array(np.where(y_train_unlabeled != -1)).shape[1]
    n_unlabelled = x_train.shape[0] - n_labelled
    randomState = np.random.RandomState(random_seed)
    idx_to_flip = randomState.choice(n_unlabelled, n_flips)
    res = [
